...
def get_pending_viabilities() -> List[Dict]:
    """Busca viabilizações pendentes (exceto FTTH aguardando busca e prédios agendados)"""
    try:
...
def get_user_results(username: str) -> List[Dict]:
    ...
    try:
        response = supabase.table('viabilizacoes')\
            .select('*')\
            .eq('usuario', username)\
            .or_('status.in.(aprovado,rejeitado,utp,pendente),status_predio.eq.aguardando_dados')\
